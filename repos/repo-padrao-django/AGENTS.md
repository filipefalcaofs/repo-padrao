...
- Algumas skills em `.claude/skills/`, `.cursor/skills/` e `.cursor/plugins/` são específicas de stack (Figma, vídeo, contagem APF). Nos **repos standalone** (`repos/repo-padrao-*`), a stack já vem aplicada — clone o repo certo em [`repos/README.md`](repos/README.md).
